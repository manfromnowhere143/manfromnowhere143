...
**[Telos](https://github.com/manfromnowhere143/telos)** — an active research program for verifying autonomous agent work by evidence, not by trust: when an agent says a long-horizon task is done, can an external protocol prove the real objective was completed — not a visible proxy — and that the agent stopped at the correct boundary? The method is a receipt-bearing completion protocol (tests, build, diff-scope checks, artifact hashes, named falsifiers, adversarial review) run as a long arc of pre-registered gates on real external trajectories from SWE-bench_Verified. The arc carries its own correction on the record: when early detector iterations were found to lack external validity — traps and detector written by the same process — verification moved off self-authored fixtures onto real ground truth.
